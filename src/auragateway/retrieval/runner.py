"""Build and verify deterministic BM25 retrieval candidates."""
...
from auragateway.retrieval.bm25 import BM25Index, to_evidence_result

_DEFAULT_SMOKE_QUERIES: Final = Path("data/retrieval/bm25-v1/smoke_queries.json")
_DEFAULT_OUTPUT_ROOT: Final = Path("data/retrieval/bm25-v1")
_DEFAULT_CHUNKING_ROOT: Final = Path("data/chunking")

FIXED_WINDOW_BM25_CONFIG: Final = RetrievalConfiguration(
    config_id="bm25-fixed-window-v1",
    chunking_config_id=FIXED_WINDOW_CONFIG.config_id,
)

SECTION_AWARE_BM25_CONFIG: Final = RetrievalConfiguration(
    config_id="bm25-section-aware-v1",
    chunking_config_id=SECTION_AWARE_CONFIG.config_id,
)


class RetrievalError(Exception):
    """Expected retrieval failure with a safe machine-readable envelope."""
...
class RetrievalErrorEnvelope(BaseModel):
    """Safe CLI failure output without document or query content."""
...
def _config_sha256(config: RetrievalConfiguration) -> str:
...
def _load_json(path: Path, not_found_code: str) -> object:
    try:
        ...
        raise RetrievalError(
            ...
            safe_message="Required retrieval input was not found.",
            path=str(path),
        ) from exc
    try:
        ...
        raise RetrievalError(
            error_code="RETRIEVAL_INPUT_INVALID_JSON",
            safe_message="Retrieval input is not valid JSON.",
            path=str(path),
        ) from exc
...
def _load_smoke_queries(path: Path) -> RetrievalSmokeQuerySet:
    try:
        return RetrievalSmokeQuerySet.model_validate(
            _load_json(path, "RETRIEVAL_SMOKE_QUERIES_NOT_FOUND")
        )
    except ValidationError as exc:
        raise RetrievalError(
            error_code="RETRIEVAL_SMOKE_QUERIES_VALIDATION_FAILED",
...
def _load_chunking_manifest(path: Path) -> ChunkingManifest:
    try:
        return ChunkingManifest.model_validate(
            _load_json(path, "RETRIEVAL_CHUNKING_MANIFEST_NOT_FOUND")
        )
    except ValidationError as exc:
        raise RetrievalError(
            error_code="RETRIEVAL_CHUNKING_MANIFEST_VALIDATION_FAILED",
            safe_message="Chunking manifest failed typed validation at the retrieval boundary.",
            path=str(path),
            details=_validation_messages(exc),
        ) from exc
...
def _load_chunks(path: Path) -> tuple[CorpusChunk, ...]:
    try:
        ...
        raise RetrievalError(
            error_code="RETRIEVAL_CHUNKS_NOT_FOUND",
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        try:
            ...
            raise RetrievalError(
                error_code="RETRIEVAL_CHUNK_VALIDATION_FAILED",
    ...
    if not chunks:
        raise RetrievalError(
            error_code="RETRIEVAL_CHUNKS_EMPTY",
...
def _smoke_results_jsonl(index: BM25Index, query_set: RetrievalSmokeQuerySet) -> bytes:
    evidence = tuple(to_evidence_result(index.search(query)) for query in query_set.queries)
    lines = [result.model_dump_json() for result in evidence]
    return ("\n".join(lines) + "\n").encode("utf-8")


def _chunking_config_for_retrieval(
    ...
    try:
        ...
        raise RetrievalError(
            error_code="RETRIEVAL_CHUNKING_CONFIG_UNSUPPORTED",
            safe_message="Retrieval configuration references an unsupported chunking candidate.",
...
def build_candidate(
...
) -> tuple[RetrievalIndexManifest, bytes]:
    """Build one sparse candidate and development smoke evidence in memory."""

    chunking_config = _chunking_config_for_retrieval(config)
    try:
        verify_chunking_candidate(repo_root, chunking_config)
    except ChunkingError as exc:
        raise RetrievalError(
            error_code="RETRIEVAL_CHUNKING_CANDIDATE_INVALID",
            safe_message="Chunking candidate verification failed before retrieval indexing.",
            path=exc.path,
    ...
    manifest = RetrievalIndexManifest(
        ...
        total_index_tokens=index.total_index_tokens,
        vocabulary_size=index.vocabulary_size,
        average_chunk_length=round(index.average_chunk_length, config.score_precision),
        smoke_query_count=len(query_set.queries),
    )
    return manifest, smoke_results


def _summary(manifest: RetrievalIndexManifest) -> RetrievalRunSummary:
    return RetrievalRunSummary(
        ...
        total_index_tokens=manifest.total_index_tokens,
        vocabulary_size=manifest.vocabulary_size,
        average_chunk_length=manifest.average_chunk_length,
...
def write_candidate(repo_root: Path, config: RetrievalConfiguration) -> RetrievalRunSummary:
    """Build and persist one candidate manifest and smoke result set."""
...
def verify_candidate(repo_root: Path, config: RetrievalConfiguration) -> RetrievalRunSummary:
    """Rebuild one candidate and compare persisted deterministic evidence."""
    ...
    try:
        persisted_manifest = RetrievalIndexManifest.model_validate(
            _load_json(manifest_path, "RETRIEVAL_MANIFEST_NOT_FOUND")
        )
    except ValidationError as exc:
        raise RetrievalError(
            error_code="RETRIEVAL_MANIFEST_VALIDATION_FAILED",
            safe_message="Persisted retrieval manifest failed typed validation.",
    ...
    try:
        ...
        raise RetrievalError(
            error_code="RETRIEVAL_SMOKE_RESULTS_NOT_FOUND",
            safe_message="Persisted smoke results were not found.",
            path=str(results_path),
        ) from exc

    if persisted_manifest != expected_manifest:
        raise RetrievalError(
            error_code="RETRIEVAL_MANIFEST_MISMATCH",
            safe_message="Persisted retrieval manifest does not match deterministic output.",
            path=str(manifest_path),
        )
    if persisted_results != expected_results:
        raise RetrievalError(
            error_code="RETRIEVAL_SMOKE_RESULTS_MISMATCH",
            safe_message="Persisted smoke results do not match deterministic output.",
            path=str(results_path),
        )
    return _summary(expected_manifest)


def build_all_candidates(repo_root: Path) -> tuple[RetrievalRunSummary, ...]:
    """Persist BM25 candidates over both Phase 1 chunking strategies."""
...
def verify_all_candidates(repo_root: Path) -> tuple[RetrievalRunSummary, ...]:
    """Verify BM25 candidates over both Phase 1 chunking strategies."""
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point for deterministic sparse retrieval candidates."""
    ...
    try:
        ...
        envelope = RetrievalErrorEnvelope(
