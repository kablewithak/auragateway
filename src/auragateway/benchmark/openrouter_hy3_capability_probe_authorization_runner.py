...
def _json_response(payload: object, *, status_code: int = 200) -> OpenRouterHttpResponse:
...
def _expect_status(status_code: int, *, retryable: bool, error_code: ProviderErrorCode) -> None:
...
def _replay_transport() -> OpenRouterProbeTransportDryRunReport:
    ...
    chat_transport = OpenRouterHttpTransport(api_key="fixture-key", backend=chat_backend)
    chat_transport.create_chat(payload={"model": "tencent/hy3:free"}, timeout_seconds=30)
    ...
    _expect_status(401, retryable=False, error_code=ProviderErrorCode.AUTHENTICATION_FAILED)
    ...
    invalid_transport = OpenRouterHttpTransport(api_key="fixture-key", backend=invalid_backend)
    try:
        invalid_transport.create_chat(payload={"model": "tencent/hy3:free"}, timeout_seconds=30)
    except LiveProviderError as exc:
...
def _validate_source_bindings(repo_root: Path, review: OpenRouterProbeAuthorizationReview) -> None:
...
def _validate_manifest(repo_root: Path, manifest: OpenRouterProbeAuthorizationManifest) -> None:
...
def validate_openrouter_probe_authorization(
    ...
    manifest = _load_model(repo_root / _MANIFEST_PATH, OpenRouterProbeAuthorizationManifest)
