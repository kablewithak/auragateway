...
NOTEBOOK_NAME: Final = "ag-b-vs-d-marker-diversified-diff-v1"
SOURCE_MAIN_COMMIT: Final = "cfe36407fe7d7b1c71938d218886872e47a1be39"
...
SCRATCH_ROOT: Final = (
    WORK_ROOT / "b_vs_d_cumulative_length_locked_marker_diversified_differential_v1_scratch"
)
...
EVIDENCE_ZIP: Final = (
    WORK_ROOT / "ag-b-vs-d-cumulative-length-locked-marker-diversified-differential-evidence-v1.zip"
)
...
def main() -> int:
    if OUTPUT_ROOT.exists() or SCRATCH_ROOT.exists() or EVIDENCE_ZIP.exists():
        raise RuntimeError("B-vs-D differential output, scratch, or evidence path already exists")
    ...
    try:
        ...
        write_json(
            OUTPUT_ROOT / "b_vs_d_marker_diversified_runtime_ready_v1.json",
            {
                "schema_version": "1.0.0",
                "status": "PASSED",
        ...
        for sequence_index, condition_id in enumerate(
            ...
            observation = run_marker_diversified_fresh_worker_observation(
                model_home,
                snapshot,
                condition_id,
                sequence_index,
            ...
            write_marker_diversified_results(results, "IN_PROGRESS")
        ...
        decision = decide_marker_diversified_differential(
            results,
            worker_reports,
            teardown_reports,
            counters,
        ...
        failure = marker_diversified_failure_record(
            error,
            active_failure_code,
            failed_stage,
            len(results),
    ...
    write_marker_diversified_results(results, result_status)

    if decision is not None and failure is None:
        write_json(
            OUTPUT_ROOT / "b_vs_d_marker_diversified_decision_v1.json",
            decision,
        )
    else:
        write_json(
    ...
    ready_path = OUTPUT_ROOT / "b_vs_d_marker_diversified_runtime_ready_v1.json"
    ...
    summary = {
        ...
        "implementation_base_commit": B_VS_D_IMPLEMENTATION_BASE_COMMIT,
        "design_record_sha256": B_VS_D_DESIGN_RECORD_SHA256,
        "authorization": authorization,
        "status": ("DIAGNOSTIC_COMPLETE" if diagnostic_valid else "DIAGNOSTIC_INVALID"),
        "decision_state": decision_state,
        "primary_endpoint": "exact_object",
        "prompt_token_count_per_condition": B_VS_D_PROMPT_TOKEN_COUNT,
        "request_order": list(B_VS_D_REQUEST_ORDER),
    ...
    write_json(
        OUTPUT_ROOT / "b_vs_d_marker_diversified_summary_v1.json",
        summary,
    )
    ...
    try:
        bundle = marker_diversified_bundle_outputs()
    except Exception as error:
        terminal = {
            **summary,
            "status": "DIAGNOSTIC_INVALID",
