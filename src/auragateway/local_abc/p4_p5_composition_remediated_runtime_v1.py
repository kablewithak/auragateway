...
SOURCE_MAIN_COMMIT: Final = "4afdcf9d840bc90ceb34af8dae098998f78de572"
...
EVIDENCE_ZIP = WORK_ROOT / "ag-p5-p6-transaction-bound-evidence-v1.zip"
...
def main() -> int:
    if OUTPUT_ROOT.exists() or SCRATCH_ROOT.exists() or EVIDENCE_ZIP.exists():
        raise RuntimeError("P5/P6 exact-runtime output or scratch path already exists")
    ...
    try:
        ...
        c3_decision = BehaviorDecision(
            ...
            state=BehaviorState.PASS,
            failure_class=None,
            reasons=("BASE_COLD completed exactly once on worker 1 generation 1",),
        )
        ...
        write_json(
            ...
            evidence_projection(
                c4_decision,
                {
                    "structured_output_sha256": cold["structured_output_sha256"],
                    "structured_output_valid": cold["structured_output_valid"],
        ...
        p6_observations = {
            ...
            "CROSS_WORKER_COLD": request_public_evidence(cross_worker_cold),
            "WORKER1_RETENTION": request_public_evidence(worker_1_retention),
            "request_counters_reconciled": True,
        }
        write_json(
    ...
    teardown_status = (
        "NOT_RUN" if not teardown_reports else "FAILED" if teardown_failures else "PASSED"
    )
    write_json(
        OUTPUT_ROOT / "worker_teardown_report_v1.json",
        {
            "schema_version": "1.0.0",
            "report_id": ("auragateway-exact-runtime-p5-p6-worker-teardown-v1"),
            "status": teardown_status,
            "worker_teardowns": teardown_reports,
            "all_capture_threads_finalized": (
                not teardown_failures
                and all(
                    bool(item.get("capture_threads_finalized", True)) for item in teardown_reports
                )
            ),
            "all_ports_closed": (
                not teardown_failures
            ...
            "all_gpu_processes_absent": (
                not teardown_failures
                and all(
                    bool(item.get("gpu_processes_absent_after", True)) for item in teardown_reports
                )
            ),
        },
    )
    ...
    post_restart_native_path = OUTPUT_ROOT / "p5_post_restart_native_origin_report_v1.json"
    ...
    source_identity_report = json.loads(
        (OUTPUT_ROOT / "runtime_source_identity_report_v1.json").read_text(encoding="utf-8")
    )
    install_report = json.loads(
        (OUTPUT_ROOT / "runtime_install_report_v1.json").read_text(encoding="utf-8")
    )
    import_closure_report = json.loads(
        (OUTPUT_ROOT / "runtime_import_closure_report_v1.json").read_text(encoding="utf-8")
    )

    summary_status = "PASSED" if passed else terminal_state.split("_", 1)[0]
    summary = {
        "schema_version": "1.0.0",
        "diagnostic_id": ("auragateway-exact-runtime-p5-p6-requalification-v2"),
        "source_main_commit": SOURCE_MAIN_COMMIT,
        "implementation_review_sha256": IMPLEMENTATION_REVIEW_SHA256,
        "design_record_sha256": DESIGN_RECORD_SHA256,
        "v5_acceptance_sha256": V5_ACCEPTANCE_SHA256,
        ...
        "failure_class": (None if failure is None else failure.get("failure_class")),
        "failed_capability": (None if failure is None else failure.get("failed_capability")),
        "p5_decision": (None if p5_decision is None else p5_decision.state.value),
        "p6_decision": (None if p6_decision is None else p6_decision.state.value),
        ...
        "runtime_import_closure_process_outcome": import_closure_report["process_outcome"],
        ...
        "public_evidence_used_as_semantic_input": False,
        "next_gate": ("PRESERVE_AND_DISPOSITION_EXACT_RUNTIME_P5_P6_REQUALIFICATION_V2"),
    }
    write_json(
    ...
    try:
        ...
        if failure is None:
            ...
            failure["secondary_evidence_packaging_error_type"] = type(error).__name__
        write_json(OUTPUT_ROOT / "failure_report_v1.json", failure)
