"""P4/P5 composition-differential transaction-bound execution authorization V1.
...
This module does not itself execute Kaggle.
"""
...
DESIGN_MERGE_COMMIT: Final = "0ae3c293b474f9a457ce06c7716121bff59af1a6"
IMPLEMENTATION_MERGE_COMMIT: Final = "96dea44afa28e1b61c68eb0eccfc91d312bb89e0"
...
DESIGN_RECORD_SHA256: Final = "f15a2926001dc4c625a6b60111269a1f3e2b6095d825455a0e8e6e0e77ee6ad2"
...
SUCCESSOR_RUNTIME_SHA256: Final = "4711f94031bc65ae159dab14412d99cfbd9ecee01b5a2d7d2fd7a2c2b09d7db7"

SOURCE_PATH: Final = Path(
    "src/auragateway/local_abc/p4_p5_composition_differential_execution_authorization_v1.py"
)
TEMPLATE_PATH: Final = Path(
    "src/auragateway/local_abc/templates/"
    "p4_p5_composition_differential_transaction_bound_wrapper_v1.py.tmpl"
)
TEST_PATH: Final = Path(
    "tests/unit/local_abc/test_p4_p5_composition_differential_execution_authorization_v1.py"
)
REPORT_PATH: Final = Path(
    "docs/reports/AuraGateway_P4_P5_Composition_Differential_Execution_Authorization_V1.md"
)
RUNBOOK_PATH: Final = Path(
    "docs/runbooks/local_abc_p4_p5_composition_differential_execution_authorization_v1.md"
)
REVIEW_PATH: Final = Path(
    "benchmarks/local_abc/"
    "auragateway_p4_p5_composition_differential_execution_authorization_v1_review.json"
)
RECORD_PATH: Final = Path(
    "benchmarks/local_abc/"
    "auragateway_p4_p5_composition_differential_execution_authorization_v1_record.json"
)

LIVE_AUTHORIZATION_PATH: Final = Path(
    "benchmarks/local_abc/"
    "auragateway_p4_p5_composition_differential_execution_authorization_v1_live.json"
)
LIVE_MANIFEST_PATH: Final = Path(
    "benchmarks/local_abc/"
    "auragateway_p4_p5_composition_differential_execution_artifact_v1_live_manifest.json"
)
TERMINAL_RECEIPT_PATH: Final = Path(
    "benchmarks/local_abc/"
    "auragateway_p4_p5_composition_differential_execution_authorization_v1_terminal.json"
)

AUTHORIZATION_SCOPE: Final = "P4_P5_COMPOSITION_DIFFERENTIAL_V1"
...
NEXT_GATE_AFTER_ISSUE: Final = "FRESH_T4_X2_INTERNET_OFF_OBSERVATION_THEN_ONE_SAVE_AND_RUN_ALL"
NEXT_GATE_AFTER_TERMINAL: Final = "PRESERVE_AND_CLASSIFY_P4_P5_COMPOSITION_DIFFERENTIAL_EVIDENCE_V1"
...
class _ArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> Never:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_ARGUMENT_ERROR",
            message,
        )
...
class DifferentialExperiment(FrozenModel):
    variable_under_test: Literal["MESSAGE_COMPOSITION_ONLY"] = "MESSAGE_COMPOSITION_ONLY"
    case_a_roles: tuple[str, ...] = ("system", "user")
    case_b_roles: tuple[str, ...] = (
        "system",
        "user",
        "assistant",
        "user",
    )
    request_order: tuple[str, ...] = ("A", "B", "B", "A", "A", "B")
    repetitions_per_case: Literal[3] = 3
    canonical_final_object: Literal['{"probe":"exact-runtime-p5-p6","value":1}'] = (
        '{"probe":"exact-runtime-p5-p6","value":1}'
    )
    a_3_of_3_b_0_of_3: Literal["COMPOSITION_REGRESSION_SUPPORTED"] = (
        "COMPOSITION_REGRESSION_SUPPORTED"
    )
    a_3_of_3_b_3_of_3: Literal["COMPOSITION_HYPOTHESIS_NOT_REPRODUCED"] = (
        "COMPOSITION_HYPOTHESIS_NOT_REPRODUCED"
    )
    a_not_3_of_3: Literal["SIMPLE_CONTROL_NOT_RELIABLE"] = "SIMPLE_CONTROL_NOT_RELIABLE"
    otherwise: Literal["NON_DETERMINISTIC_OR_AMBIGUOUS"] = "NON_DETERMINISTIC_OR_AMBIGUOUS"
    mixed_result_requires_separate_case_c_design: Literal[True] = True
    case_c_authorized_by_this_design: Literal[False] = False
    runtime_remediation_authorized: Literal[False] = False

    @model_validator(mode="after")
    def validate_contract(self) -> Self:
        if self.case_a_roles != ("system", "user"):
            raise ValueError("Case A role contract drifted")
        if self.case_b_roles != (
            "system",
            "user",
            "assistant",
            "user",
        ):
            raise ValueError("Case B role contract drifted")
        if self.request_order != ("A", "B", "B", "A", "A", "B"):
...
class AuthorizationIntent(FrozenModel):
    ...
    scope: Literal["P4_P5_COMPOSITION_DIFFERENTIAL_V1"]
    ...
    implementation_merge_commit: Literal["96dea44afa28e1b61c68eb0eccfc91d312bb89e0"]
...
class AuthorizationBody(FrozenModel):
    schema_version: Literal["1.0.0"] = "1.0.0"
    authorization_id: str = Field(pattern=r"^p4p5-[0-9a-f]{32}$")
...
class TerminalReceipt(FrozenModel):
    ...
    platform_observation: PlatformObservation | None = None
    ...
    def validate_terminal_semantics(self) -> Self:
        ...
        if self.platform_observation is None:
            raise ValueError("attempted disposition requires platform observation")
...
def _read_required(root: Path, relative: Path) -> bytes:
    ...
    if not path.is_file() or path.is_symlink():
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_ARTIFACT_MISSING",
...
def _read_json_object(root: Path, relative: Path) -> dict[str, object]:
    ...
    if not isinstance(parsed, dict):
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_JSON_INVALID",
...
def _require_git(root: Path, *arguments: str) -> str:
    ...
    if code != 0:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_GIT_STATE_FAILED",
...
def _require_design_ancestor(root: Path) -> None:
    ...
    if code != 0:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_NOT_ANCESTOR",
...
def _require_merged_clean_main(root: Path) -> str:
    ...
    if branch != "main":
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_NOT_ON_MAIN",
    ...
    if head != origin_main:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_MAIN_NOT_SYNCHRONIZED",
    ...
    if status:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_REPOSITORY_NOT_CLEAN",
    ...
    if code != 0:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_NOT_MERGED",
...
def _verify_hash(
    ...
    if observed != expected_sha256:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_IDENTITY_DRIFT",
...
def _validate_design(root: Path) -> None:
    ...
    if not isinstance(parsed, dict):
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_INVALID",
    ...
    if drift:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_CONTRACT_DRIFT",
            "authorization design contract drifted",
            DESIGN_RECORD_PATH.as_posix(),
        )

    budget = record.get("execution_budget")
    if budget != ExecutionBudget().model_dump(mode="json"):
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_BUDGET_DRIFT",
            "authorization design execution budget drifted",
            DESIGN_RECORD_PATH.as_posix(),
        )

    runtime = record.get("runtime_model")
    if runtime != RuntimeModelContract().model_dump(mode="json"):
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_RUNTIME_CONTRACT_DRIFT",
            "authorization design runtime/model contract drifted",
            DESIGN_RECORD_PATH.as_posix(),
        )

    experiment = record.get("experiment")
    if experiment != DifferentialExperiment().model_dump(mode="json"):
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_EXPERIMENT_DRIFT",
            "authorization design differential contract drifted",
            DESIGN_RECORD_PATH.as_posix(),
        )

    binding = record.get("authorization_payload_binding")
    expected_binding = {
        "authorization_scope_bound": True,
        "authorization_design_record_sha256_bound": True,
        "issuer_merge_commit_bound": True,
        "implementation_merge_commit_bound": True,
        "implementation_authority_hashes_bound": True,
        "runtime_model_contract_bound": True,
        "execution_budget_bound": True,
        "differential_experiment_contract_bound": True,
        "required_platform_policy_bound": True,
        "authorization_window_bound": True,
    }

    if binding != expected_binding:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_PAYLOAD_BINDING_DRIFT",
            "authorization payload binding contract drifted",
            DESIGN_RECORD_PATH.as_posix(),
        )
...
def _static_review(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "review_id": (
            "auragateway-p4-p5-composition-differential-execution-authorization-v1-review"
        ),
...
def _static_record(
    ...
    return {
        "schema_version": "1.0.0",
        "record_id": ("auragateway-p4-p5-composition-differential-execution-authorization-v1"),
...
def generate_static(root: Path) -> dict[str, object]:
    ...
    return {
        "status": "P4_P5_DIFF_AUTHORIZATION_STATIC_ARTIFACTS_GENERATED",
        "review_sha256": _sha256(review_bytes),
...
def validate_static(root: Path) -> dict[str, object]:
    ...
    for relative, expected in (
        ...
        if observed != expected:
            raise AuthorizationIssuerError(
                "P4_P5_DIFF_AUTHORIZATION_STATIC_ARTIFACT_DRIFT",
    ...
    for relative in (
        ...
        if (root / relative).exists():
            raise AuthorizationIssuerError(
                "P4_P5_DIFF_AUTHORIZATION_LIVE_LIFECYCLE_PRESENT",
                "static validation requires no live lifecycle artifact",
                relative.as_posix(),
            )

    return {
        "status": "P4_P5_DIFF_EXECUTION_AUTHORIZATION_V1_VALID",
        "authorization_scope": AUTHORIZATION_SCOPE,
        "maximum_model_requests": 6,
        "maximum_worker_starts": 1,
        "maximum_model_loads": 1,
...
def build_authorization(
    ...
    *,
    ...
    if challenge != expected_challenge:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_CHALLENGE_DRIFT",
            "authorization challenge does not bind exact intent",
        )

    confirmed = _normalize_time(
        confirmed_at,
        "confirmed_at",
    )
    ...
    if confirmed < intent.prepared_at:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_CONFIRMATION_TIME_INVALID",
    ...
    if confirmed - intent.prepared_at > timedelta(minutes=MAX_CONFIRMATION_AGE_MINUTES):
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_CONFIRMATION_STALE",
            "operator confirmation exceeded freshness window",
        )

    body = AuthorizationBody(
        authorization_id="p4p5-" + intent.intent_id,
        scope=AUTHORIZATION_SCOPE,
...
def render_executable_payload(
    ...
    if authorization.authorization.generator_contract_sha256 != generator_sha:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_GENERATOR_DRIFT",
    ...
    if authorization.authorization.issuer_source_sha256 != issuer_source_sha:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_ISSUER_SOURCE_DRIFT",
    ...
    if authorization.authorization.runtime_payload_sha256 != _sha256(runtime_payload):
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_RUNTIME_PAYLOAD_DRIFT",
    ...
    for marker, value in replacements.items():
        if source.count(marker) != 1:
            raise AuthorizationIssuerError(
                "P4_P5_DIFF_AUTHORIZATION_TEMPLATE_MARKER_DRIFT",
...
def authorize_generate(
    ...
    *,
    ...
    for relative in (
        ...
        if (root / relative).exists():
            raise AuthorizationIssuerError(
                "P4_P5_DIFF_AUTHORIZATION_LIFECYCLE_EXISTS",
    ...
    if output_path.exists():
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_OUTPUT_EXISTS",
    ...
    print("maximum_worker_starts=1")
    print("maximum_model_loads=1")
    print("maximum_hidden_retries=0")
    print("required_platform=T4_X2 / 2 GPUs / Internet Off")

    observed = input(
        "Retype the authorization challenge to authorize exactly one P4/P5 differential execution: "
    ).strip()

    if observed != challenge:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_CONFIRMATION_MISMATCH",
    ...
    return {
        "status": ("P4_P5_DIFF_EXECUTION_ARTIFACT_AUTHORIZED_AND_GENERATED"),
        "transaction_id": authorization.transaction_id,
        "authorization_sha256": _sha256(authorization_bytes),
...
def terminalize(
    ...
    *,
    ...
    if (root / TERMINAL_RECEIPT_PATH).exists():
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_ALREADY_TERMINAL",
            "transaction already has a terminal receipt",
        )

    authorization_artifact = _read_required(
        root,
        LIVE_AUTHORIZATION_PATH,
    )
    manifest_object = _read_json_object(
        root,
        LIVE_MANIFEST_PATH,
    )
    authorization_object = _read_json_object(
        root,
        LIVE_AUTHORIZATION_PATH,
    )

    authorization = ExecutionAuthorization.model_validate(authorization_object)
    manifest = ExecutionArtifactManifest.model_validate(manifest_object)
    ...
    platform: PlatformObservation | None = None

    if platform_observed_at is not None:
        platform = PlatformObservation(
            observed_at=platform_observed_at,
        )

        if platform.observed_at < authorization.authorization.issued_at:
            raise AuthorizationIssuerError(
                "P4_P5_DIFF_AUTHORIZATION_PLATFORM_TIME_INVALID",
                "platform observation precedes authorization",
            )

        if platform.observed_at >= authorization.authorization.expires_at:
            raise AuthorizationIssuerError(
                "P4_P5_DIFF_AUTHORIZATION_PLATFORM_TIME_EXPIRED",
    ...
    if not authorization_artifact:
        raise AuthorizationIssuerError(
            "P4_P5_DIFF_AUTHORIZATION_EMPTY_LIVE_ARTIFACT",
            "live authorization artifact is unexpectedly empty",
        )

    return {
        "status": "P4_P5_DIFF_EXECUTION_AUTHORIZATION_TERMINAL",
...
def _default_output() -> Path:
    return Path.home() / "Desktop" / "ag-p4-p5-composition-differential-v1.ipynb"
    ...
    try:
        ...
        if args.command == "terminalize":
            if args.disposition is None:
                raise AuthorizationIssuerError(
                    "P4_P5_DIFF_AUTHORIZATION_ARGUMENT_MISSING",
            ...
            observed_at = (
                ...
                else datetime.fromisoformat(args.platform_observed_at)
            )
        ...
        if not isinstance(
            ...
            _print_error(
                AuthorizationIssuerError(
                    "P4_P5_DIFF_AUTHORIZATION_VALIDATION_FAILED",
                    str(error),
                )
            )
