"""Single-use execution authorization for final offline verifier V3."""
...
IMPLEMENTATION_FEATURE_COMMIT: Final = "7a1c97e6c37631739a6fb1dbd306d41c91ccd9e3"
IMPLEMENTATION_MERGE_COMMIT: Final = "51402a453e1dcd84ff5933a04c37c77c40d9f603"

ISSUER_SOURCE_PATH: Final = Path(
    "src/auragateway/local_abc/"
    "preflight_v3_exact_runtime_offline_compatibility_v3_execution_authorization_v1.py"
)
ISSUER_TEST_PATH: Final = Path(
    "tests/unit/local_abc/"
    "test_preflight_v3_exact_runtime_offline_compatibility_v3_execution_authorization_v1.py"
)
ISSUER_RUNBOOK_PATH: Final = Path(
    "docs/runbooks/local_abc_preflight_v3_final_offline_verifier_v3_execution_authorization_v1.md"
)
ISSUER_RECORD_PATH: Final = Path(
    "benchmarks/local_abc/"
    "auragateway_preflight_v3_exact_runtime_offline_compatibility_v3_"
...
V3_ARTIFACT_IDENTITIES: Final[dict[str, tuple[str, int]]] = {
    ...
    (
        ...
        "5b48904000191aeb63c5de8807b48ac730ce04a730973049135f46d5fc9778d9",
        4456,
    ),
    (
        "benchmarks/local_abc/"
        ...
        "d8d217f7bd948eadb0383532777ad3c8c3dfee4909868fdf5afa5fda334c1e63",
        2320,
    ),
}

AUTHORIZATION_ID: Final = (
    "auragateway-preflight-v3-exact-runtime-offline-compatibility-v3-execution-authorization-v1"
)
...
CONFIRMATION_PHRASE: Final = (
    ...
    "FINAL_OFFLINE_VERIFIER_V3_EXECUTION"
)
...
NEXT_GATE_AFTER_ISSUE: Final = "execute_one_governed_final_offline_verifier_v3_saved_version"
NEXT_GATE_AFTER_CONSUMPTION: Final = (
    "preserve_and_accept_or_classify_final_offline_verifier_v3_evidence"
)
NEXT_GATE_AFTER_ABANDONMENT: Final = (
    "reconcile_then_issue_fresh_final_offline_verifier_v3_authorization"
)
...
class IssuerRecord(LocalABCContract):
    ...
    record_id: Literal[
        "auragateway-preflight-v3-final-offline-verifier-v3-execution-authorization-issuer-v1"
    ]
    status: Literal["FINAL_OFFLINE_VERIFIER_V3_EXECUTION_AUTHORIZATION_ISSUER_VALID"]
    bound_implementation_feature_commit: Literal["7a1c97e6c37631739a6fb1dbd306d41c91ccd9e3"]
    bound_implementation_merge_commit: Literal["51402a453e1dcd84ff5933a04c37c77c40d9f603"]
...
class ExecutionAuthorization(LocalABCContract):
    ...
    implementation_feature_commit: Literal["7a1c97e6c37631739a6fb1dbd306d41c91ccd9e3"]
    implementation_merge_commit: Literal["51402a453e1dcd84ff5933a04c37c77c40d9f603"]
    notebook_sha256: Literal["c12d0709d78b0491910a7e989b89ff1ecb69c82971df442ccebefc8fcb3e1469"]
    ...
    offline_verifier_v3_execution_authorized: Literal[True]
    model_execution_authorized: Literal[False]
    p5_p6_execution_authorized: Literal[False]
    pilot_execution_authorized: Literal[False]
    final_measured_abc_execution_authorized: Literal[False]
    authorization_reusable: Literal[False]
    next_gate: Literal["execute_one_governed_final_offline_verifier_v3_saved_version"]
...
class AuthorizationConsumption(LocalABCContract):
    ...
    authorization_reusable: Literal[False]
    offline_verifier_v3_execution_authorized: Literal[False]
    model_execution_authorized: Literal[False]
    p5_p6_execution_authorized: Literal[False]
    pilot_execution_authorized: Literal[False]
    final_measured_abc_execution_authorized: Literal[False]
    next_gate: Literal["preserve_and_accept_or_classify_final_offline_verifier_v3_evidence"]
...
class AuthorizationAbandonment(LocalABCContract):
    ...
    pilot_execution_authorized: Literal[False]
    final_measured_abc_execution_authorized: Literal[False]
    next_gate: Literal["reconcile_then_issue_fresh_final_offline_verifier_v3_authorization"]
...
def _identity(repo_root: Path, path: Path) -> ArtifactIdentity:
    ...
    if not target.is_file():
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTH_ISSUER_ARTIFACT_MISSING",
...
def _require_bound_implementation(repo_root: Path) -> tuple[ArtifactIdentity, ...]:
    ...
    if returncode != 0:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTH_ISSUER_IMPLEMENTATION_NOT_ANCESTOR",
            "bound V3 implementation merge commit is not an ancestor of HEAD",
        )

    identities: list[ArtifactIdentity] = []
    for path_text, (expected_sha, expected_size) in V3_ARTIFACT_IDENTITIES.items():
        path = Path(path_text)
        identity = _identity(repo_root, path)
        if identity.sha256 != expected_sha or identity.size_bytes != expected_size:
            _raise(
                "FINAL_OFFLINE_VERIFIER_V3_AUTH_ISSUER_IMPLEMENTATION_DRIFT",
                "bound V3 implementation artifact identity drifted",
                path,
                (
...
def build_record(repo_root: Path) -> IssuerRecord:
    ...
    return IssuerRecord(
        ...
        status="FINAL_OFFLINE_VERIFIER_V3_EXECUTION_AUTHORIZATION_ISSUER_VALID",
...
def validate_implementation(repo_root: Path) -> dict[str, object]:
    ...
    if not target.is_file():
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTH_ISSUER_RECORD_MISSING",
            "issuer record is missing",
            ISSUER_RECORD_PATH,
        )
    if target.read_bytes() != expected_bytes:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTH_ISSUER_RECORD_DRIFT",
...
def _require_no_terminal_or_live_artifact(repo_root: Path) -> None:
    ...
    if present:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_LIFECYCLE_ALREADY_STARTED",
...
def _require_issue_repo_state(repo_root: Path, issuer_merge_commit: str) -> None:
    branch = _require_git_success(
        repo_root,
        "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_GIT_STATE_FAILED",
    ...
    if branch != "main":
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_NOT_ON_MAIN",
    ...
    if head != issuer_merge_commit:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_ISSUER_COMMIT_MISMATCH",
            "confirmed issuer merge commit does not equal HEAD",
        )
    if head != origin_main:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_MAIN_NOT_SYNCHRONIZED",
    ...
    if status:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_REPOSITORY_NOT_CLEAN",
    ...
    if returncode != 0:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_IMPLEMENTATION_NOT_ANCESTOR",
            "bound V3 implementation is not contained by issuer main",
        )
...
def issue_authorization(
    ...
    *,
    ...
    authorization = ExecutionAuthorization(
        ...
        notebook_sha256=V3_ARTIFACT_IDENTITIES[
            "notebooks/auragateway_preflight_v3_exact_runtime_offline_compatibility_v3.ipynb"
        ][0],
        ...
        offline_verifier_v3_execution_authorized=True,
...
def _read_authorization(repo_root: Path) -> tuple[ExecutionAuthorization, bytes]:
    ...
    if not target.is_file():
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_MISSING",
    ...
    try:
        ...
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_INVALID",
    ...
    if payload != canonical:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_NONCANONICAL",
...
def validate_live_authorization(
    ...
    *,
    ...
    if (repo_root / CONSUMPTION_PATH).exists() or (repo_root / ABANDONMENT_PATH).exists():
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_ALREADY_TERMINAL",
    ...
    if observed_now >= authorization.expires_at:
        _raise(
            "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_EXPIRED",
...
def consume_authorization(
    ...
    *,
    ...
    receipt = AuthorizationConsumption(
        ...
        authorization_reusable=False,
        offline_verifier_v3_execution_authorized=False,
        model_execution_authorized=False,
        p5_p6_execution_authorized=False,
        pilot_execution_authorized=False,
        final_measured_abc_execution_authorized=False,
    ...
    try:
        if args.command == "generate-record":
            ...
            if args.issuer_merge_commit is None:
                _raise(
                    "FINAL_OFFLINE_VERIFIER_V3_AUTHORIZATION_ARGUMENT_MISSING",
