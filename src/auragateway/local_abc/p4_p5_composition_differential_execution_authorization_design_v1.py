"""Freeze the P4/P5 composition-differential execution authorization design V1.
...
composition-differential implementation. It does not issue authority, generate
an executable notebook, load a model, start a worker, or perform a request.
"""
...
BASE_MAIN_COMMIT: Final = "96dea44afa28e1b61c68eb0eccfc91d312bb89e0"
...
class _ArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> Never:
        raise AuthorizationDesignError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_ARGUMENT_ERROR",
            message,
        )
...
class DifferentialExperimentContract(FrozenModel):
    ...
    def validate_order(self) -> Self:
        if self.request_order != ("A", "B", "B", "A", "A", "B"):
            raise ValueError("differential request order drifted")
        if self.request_order.count("A") != 3:
            raise ValueError("Case A repetition count drifted")
        if self.request_order.count("B") != 3:
            raise ValueError("Case B repetition count drifted")
        return self


class EvidenceContract(FrozenModel):
    expected_evidence_zip: Literal["ag-p4-p5-composition-differential-evidence-v1.zip"] = (
...
class AuthorizationDesignRecord(FrozenModel):
    ...
    base_main_commit: Literal["96dea44afa28e1b61c68eb0eccfc91d312bb89e0"]
    ...
    authorization_scope: Literal["P4_P5_COMPOSITION_DIFFERENTIAL_V1"]
...
def _read_authority(
    ...
    if observed != expected_sha256:
        raise AuthorizationDesignError(
            "P4_P5_DIFF_AUTHORITY_IDENTITY_DRIFT",
...
def _require_base_ancestor(root: Path) -> None:
    ...
    if completed.returncode != 0:
        raise AuthorizationDesignError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_ANCESTRY_DRIFT",
...
def build_record(root: Path) -> AuthorizationDesignRecord:
    ...
    return AuthorizationDesignRecord(
        design_id=("auragateway-p4-p5-composition-differential-execution-authorization-design-v1"),
...
def generate(root: Path) -> dict[str, object]:
    ...
    return {
        "status": "P4_P5_DIFF_AUTHORIZATION_DESIGN_GENERATED",
...
def validate(root: Path) -> dict[str, object]:
    ...
    if not path.is_file() or path.is_symlink():
        raise AuthorizationDesignError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_RECORD_MISSING",
    ...
    if observed != expected:
        raise AuthorizationDesignError(
            "P4_P5_DIFF_AUTHORIZATION_DESIGN_RECORD_DRIFT",
    ...
    return {
        "status": "P4_P5_DIFF_AUTHORIZATION_DESIGN_VALID",
        ...
        "issuer_merge_commit_binding_required": (
            record.authorization_payload_binding.issuer_merge_commit_bound
        ),
        "runtime_model_contract_bound": (
            record.authorization_payload_binding.runtime_model_contract_bound
        ),
...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(error, AuthorizationDesignError):
            code = "P4_P5_DIFF_AUTHORIZATION_DESIGN_VALIDATION_FAILED"
            message = str(error)
            path = None
