"""Freeze P4/P5 composition-remediation execution authorization design V1.
...
remediation implementation and its full P5/P6 confirmation contract.
...
BASE_MAIN_COMMIT: Final = "f5701274037162ab9ff8f0627a544ac76d9c1b7b"
...
DESIGN_RECORD_PATH: Final = Path(
    ...
    "auragateway_p4_p5_composition_remediation_execution_authorization_design_v1.json"
)
...
class _ArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> Never:
        raise AuthorizationDesignError(
            "P4_P5_REMEDIATION_AUTHORIZATION_DESIGN_ARGUMENT_ERROR",
            message,
        )
...
class QualificationContract(FrozenModel):
    ...
    def validate_request_roles(self) -> Self:
        if len(self.structured_request_roles) != self.structured_request_count:
            raise ValueError("structured-request count drifted")
        if len(set(self.structured_request_roles)) != self.structured_request_count:
            raise ValueError("structured-request roles are not unique")
        return self


class EvidenceContract(FrozenModel):
    expected_evidence_zip: Literal["ag-p5-p6-transaction-bound-evidence-v1.zip"] = (
...
class AuthorizationDesignRecord(FrozenModel):
    ...
    base_main_commit: Literal["f5701274037162ab9ff8f0627a544ac76d9c1b7b"]
    ...
    authorization_scope: Literal["P4_P5_COMPOSITION_REMEDIATION_CONFIRMATION_V1"]
...
def _read_authority(
    ...
    if observed != expected_sha256:
        raise AuthorizationDesignError(
            "P4_P5_REMEDIATION_AUTHORITY_IDENTITY_DRIFT",
...
def _require_base_ancestor(root: Path) -> None:
    ...
    if completed.returncode != 0:
        raise AuthorizationDesignError(
            "P4_P5_REMEDIATION_AUTHORIZATION_DESIGN_ANCESTRY_DRIFT",
...
def build_record(root: Path) -> AuthorizationDesignRecord:
    ...
    return AuthorizationDesignRecord(
        design_id=("auragateway-p4-p5-composition-remediation-execution-authorization-design-v1"),
...
def generate(root: Path) -> dict[str, object]:
    ...
    return {
        "status": "P4_P5_REMEDIATION_AUTHORIZATION_DESIGN_GENERATED",
...
def validate(root: Path) -> dict[str, object]:
    ...
    if not path.is_file() or path.is_symlink():
        raise AuthorizationDesignError(
            "P4_P5_REMEDIATION_AUTHORIZATION_DESIGN_RECORD_MISSING",
    ...
    if observed != expected:
        raise AuthorizationDesignError(
            "P4_P5_REMEDIATION_AUTHORIZATION_DESIGN_RECORD_DRIFT",
    ...
    return {
        "status": "P4_P5_REMEDIATION_AUTHORIZATION_DESIGN_VALID",
        ...
        "structured_request_count": record.qualification.structured_request_count,
        "p5_state_required": record.qualification.p5_state_required,
        "p6_state_required": record.qualification.p6_state_required,
...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(error, AuthorizationDesignError):
            code = "P4_P5_REMEDIATION_AUTHORIZATION_DESIGN_VALIDATION_FAILED"
            message = str(error)
            path = None
