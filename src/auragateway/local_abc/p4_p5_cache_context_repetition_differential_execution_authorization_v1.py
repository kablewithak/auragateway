"""P4/P5 cache-context repetition differential transaction-bound execution authorization V1.
...
DESIGN_MERGE_COMMIT: Final = "0ad27e48e72f91f52ca48927a66bbe44f099e258"
IMPLEMENTATION_MERGE_COMMIT: Final = "658a21516fa6b1cc72bd53c2c65e51aae88b4d79"
...
DESIGN_RECORD_SHA256: Final = "900b76c0cf8f833733f63c006e4aa489f9581d80260f4f30f6a4b9161c973a77"
...
SOURCE_PATH: Final = Path(
    ...
    "p4_p5_cache_context_repetition_differential_execution_authorization_v1.py"
)
...
TEST_PATH: Final = Path(
    ...
    "test_p4_p5_cache_context_repetition_differential_execution_authorization_v1.py"
)
REPORT_PATH: Final = Path(
    "docs/reports/"
    "AuraGateway_P4_P5_Cache_Context_Repetition_Differential_Execution_Authorization_V1.md"
)
RUNBOOK_PATH: Final = Path(
    "docs/runbooks/"
    "local_abc_p4_p5_cache_context_repetition_differential_execution_authorization_v1.md"
)
...
NEXT_GATE_AFTER_OBSERVATION: Final = (
    "ONE_SAVE_AND_RUN_ALL_P4_P5_CACHE_CONTEXT_REPETITION_DIFFERENTIAL_V1"
)
NEXT_GATE_AFTER_TERMINAL: Final = (
    "PRESERVE_AND_CLASSIFY_P4_P5_CACHE_CONTEXT_REPETITION_DIFFERENTIAL_EVIDENCE_V1"
)
...
class _ArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> Never:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_ARGUMENT_ERROR", message
        )
...
class DifferentialExperiment(FrozenModel):
    ...
    def validate_contract(self) -> Self:
        ...
        if self.request_order.count(self.control_condition_id) != self.observations_per_condition:
            raise ValueError("CONTROL_1X observation count drifted")
        if self.request_order.count(self.treatment_condition_id) != self.observations_per_condition:
...
class AuthorizationIntent(FrozenModel):
    ...
    scope: Literal["P4_P5_CACHE_CONTEXT_REPETITION_DIFFERENTIAL_V1"]
...
class AuthorizationBody(FrozenModel):
    schema_version: Literal["1.0.0"] = "1.0.0"
    authorization_id: str = Field(pattern=r"^p4p5rep-[0-9a-f]{32}$")
...
def _read_required(root: Path, relative: Path) -> bytes:
    ...
    if not path.is_file() or path.is_symlink():
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_ARTIFACT_MISSING",
...
def _read_json_object(root: Path, relative: Path) -> dict[str, object]:
    ...
    if not isinstance(parsed, dict):
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_JSON_INVALID",
...
def _require_git(root: Path, *arguments: str) -> str:
    ...
    if code != 0:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_GIT_STATE_FAILED",
...
def _require_design_ancestor(root: Path) -> None:
    ...
    if code != 0:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_DESIGN_NOT_ANCESTOR",
...
def _require_merged_clean_main(root: Path) -> str:
    ...
    if branch != "main":
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_NOT_ON_MAIN",
    ...
    if head != origin_main:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_MAIN_NOT_SYNCHRONIZED",
            "HEAD must equal origin/main before live authorization",
        )
    status = _require_git(root, "status", "--porcelain=v1", "-uall")
    if status:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_REPOSITORY_NOT_CLEAN",
            "repository must be clean before live authorization",
        )
    code, _, _ = _git(root, "merge-base", "--is-ancestor", DESIGN_MERGE_COMMIT, head)
    if code != 0:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_DESIGN_NOT_MERGED",
...
def _verify_hash(root: Path, relative: Path, expected_sha256: str) -> bytes:
    ...
    if _sha256(payload) != expected_sha256:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_IDENTITY_DRIFT",
...
def _validate_design(root: Path) -> None:
    ...
    if not isinstance(parsed, dict):
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_DESIGN_INVALID",
    ...
    if not isinstance(observation, dict) or observation.get("control_id") != PLATFORM_CONTROL_ID:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_PLATFORM_CONTROL_DRIFT",
...
def generate_static(root: Path) -> dict[str, object]:
    ...
    return {
        "status": "P4_P5_REPETITION_DIFF_AUTHORIZATION_STATIC_ARTIFACTS_GENERATED",
        "review_sha256": _sha256(review_bytes),
...
def validate_static(root: Path) -> dict[str, object]:
    ...
    for relative, expected in ((REVIEW_PATH, review_bytes), (RECORD_PATH, record_bytes)):
        if _read_required(root, relative) != expected:
            raise AuthorizationIssuerError(
                "P4_P5_REPETITION_DIFF_AUTHORIZATION_STATIC_ARTIFACT_DRIFT",
    ...
    for relative in (
        ...
        if (root / relative).exists():
            raise AuthorizationIssuerError(
                "P4_P5_REPETITION_DIFF_AUTHORIZATION_LIVE_LIFECYCLE_PRESENT",
                "static validation requires no live lifecycle artifact",
                relative.as_posix(),
            )
    return {
        "status": "P4_P5_REPETITION_DIFF_EXECUTION_AUTHORIZATION_V1_VALID",
        "authorization_scope": AUTHORIZATION_SCOPE,
        "maximum_model_requests": 6,
        "maximum_worker_starts": 6,
        "maximum_model_loads": 6,
...
def build_authorization(
    ...
    if challenge != expected_challenge:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_CHALLENGE_DRIFT",
    ...
    if confirmed < intent.prepared_at:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_CONFIRMATION_TIME_INVALID",
    ...
    if confirmed - intent.prepared_at > timedelta(minutes=MAX_CONFIRMATION_AGE_MINUTES):
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_CONFIRMATION_STALE",
            "operator confirmation exceeded freshness window",
        )
    body = AuthorizationBody(
        authorization_id="p4p5rep-" + intent.intent_id,
        scope=AUTHORIZATION_SCOPE,
...
def render_executable_payload(
    ...
    if authorization.authorization.generator_contract_sha256 != generator_sha:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_GENERATOR_DRIFT",
    ...
    if authorization.authorization.issuer_source_sha256 != issuer_source_sha:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_ISSUER_SOURCE_DRIFT",
    ...
    if authorization.authorization.runtime_payload_sha256 != _sha256(runtime_payload):
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_RUNTIME_PAYLOAD_DRIFT",
    ...
    for marker, value in replacements.items():
        if source.count(marker) != 1:
            raise AuthorizationIssuerError(
                "P4_P5_REPETITION_DIFF_AUTHORIZATION_TEMPLATE_MARKER_DRIFT",
...
def authorize_generate(root: Path, output_path: Path, *, window_minutes: int) -> dict[str, object]:
    ...
    for relative in (
        ...
        if (root / relative).exists():
            raise AuthorizationIssuerError(
                "P4_P5_REPETITION_DIFF_AUTHORIZATION_LIFECYCLE_EXISTS",
    ...
    if output_path.exists():
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_OUTPUT_EXISTS",
    ...
    print("variable_under_test=CACHE_CONTEXT_REPETITION_COUNT")
    print("control_repetition_count=1")
    print("treatment_repetition_count=24")
    ...
    observed = input(
        ...
        "P4/P5 cache-context repetition differential execution: "
    ).strip()
    if observed != challenge:
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_CONFIRMATION_MISMATCH",
    ...
    return {
        "status": "P4_P5_REPETITION_DIFF_EXECUTION_ARTIFACT_AUTHORIZED_AND_GENERATED",
        "transaction_id": authorization.transaction_id,
        "authorization_sha256": _sha256(authorization_bytes),
...
def record_platform_observation(root: Path, *, observed_at: datetime) -> dict[str, object]:
    ...
    if (root / TERMINAL_RECEIPT_PATH).exists():
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_AUTHORIZATION_ALREADY_TERMINAL",
            "transaction already has a terminal receipt",
        )
    if (root / PLATFORM_OBSERVATION_RECEIPT_PATH).exists():
        raise AuthorizationIssuerError(
            "P4_P5_REPETITION_DIFF_PLATFORM_OBSERVATION_ALREADY_PERSISTED",
    ...
    return {
        "status": "P4_P5_REPETITION_DIFF_PLATFORM_OBSERVATION_PERSISTED",
    ...
    *,
    ...
    if (root / PLATFORM_OBSERVATION_RECEIPT_PATH).is_file():
        ...
        if platform.transaction_id != authorization.transaction_id:
            raise AuthorizationIssuerError(
                "P4_P5_REPETITION_DIFF_PLATFORM_TRANSACTION_DRIFT",
    ...
    return {
        "status": "P4_P5_REPETITION_DIFF_EXECUTION_AUTHORIZATION_TERMINAL",
        "transaction_id": receipt.transaction_id,
...
def _default_output() -> Path:
    return Path.home() / "Desktop" / "ag-p4-p5-cache-context-repetition-differential-v1.ipynb"
    ...
    try:
        ...
        if args.command == "record-platform-observation":
            if args.platform_observed_at is None:
                raise AuthorizationIssuerError(
                    "P4_P5_REPETITION_DIFF_AUTHORIZATION_ARGUMENT_MISSING",
        ...
        if not isinstance(error, AuthorizationIssuerError):
            _print_error(
                AuthorizationIssuerError(
                    "P4_P5_REPETITION_DIFF_AUTHORIZATION_VALIDATION_FAILED", str(error)
                )
            )
        return 2
