"""Typed contracts for remediated held-out v2 validation and Gate 1 freeze."""
...
from auragateway.contracts.retrieval_selection import MetadataPolicy, RetrievalSelectionVariant
...
class HeldOutV2FreezeRecord(BaseModel):
    """Evidence that held-out v2 was frozen before remediated candidate scoring."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: str = "1.0.0"
    record_id: str = "nimbus-relay-held-out-freeze-v2"
    ...
    held_out_v1_set_path: str
    held_out_v1_set_sha256: str
    held_out_v1_decision_path: str
    held_out_v1_decision_sha256: str
    remediation_report_path: str
    remediation_report_sha256: str
    ...
    def validate_record(self) -> HeldOutV2FreezeRecord:
        for name, value in (
            ...
            ("held_out_v1_set_sha256", self.held_out_v1_set_sha256),
            ("held_out_v1_decision_sha256", self.held_out_v1_decision_sha256),
            ("remediation_report_sha256", self.remediation_report_sha256),
            ("source_metadata_sha256", self.source_metadata_sha256),
        ):
            _validate_sha256(value, name)
        if not self.authoring_complete:
            raise ValueError("held-out v2 authoring must be complete before freeze")
        if self.candidate_results_present_at_freeze:
            raise ValueError("candidate results must not exist when held-out v2 is frozen")
        return self


class HeldOutV2Finalist(BaseModel):
    """One development-v2 finalist admitted to held-out v2 validation."""
    ...
    @model_validator(mode="after")
    def validate_finalist(self) -> HeldOutV2Finalist:
        _validate_sha256(self.retriever_config_sha256, "retriever_config_sha256")
        _validate_sha256(self.remediation_manifest_sha256, "remediation_manifest_sha256")
        if self.metadata_policy is not MetadataPolicy.AUTHORED:
            raise ValueError("held-out v2 finalists must use the authored metadata policy")
        return self


class HeldOutV2ValidationPolicy(BaseModel):
    """Frozen held-out v2 comparison policy for remediated finalists."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: str = "1.0.0"
    policy_id: str = "retrieval-held-out-validation-v2"
    status: str = "frozen_before_candidate_evaluation"
    ...
    finalists: tuple[HeldOutV2Finalist, ...] = Field(min_length=2, max_length=2)
    ...
    @model_validator(mode="after")
    def validate_policy(self) -> HeldOutV2ValidationPolicy:
        for name, value in (
        ...
        duplicates = sorted(value for value, count in Counter(config_ids).items() if count > 1)
        if duplicates:
            raise ValueError("duplicate held-out v2 finalist config IDs: " + ", ".join(duplicates))
        if sorted(ranks) != [1, 2] or len(set(ranks)) != 2:
            raise ValueError("held-out v2 finalists must be unique development ranks one and two")
        if len({item.top_k for item in self.finalists}) != 1:
            raise ValueError("held-out v2 finalists must use the same top_k")
        if not self.development_recommendation_is_not_auto_confirmed:
            raise ValueError("development-v2 recommendation must not be auto-confirmed")
        if not self.retrieval_freeze_requires_gate_pass:
            raise ValueError("retrieval freeze must require a held-out v2 hard-gate pass")
        if self.measured_execution_permitted:
            raise ValueError("Gate 1 policy cannot permit measured runtime execution")
        return self


class HeldOutV2ScorecardReference(BaseModel):
    """Hash reference to one held-out v2 finalist scorecard and result set."""
    ...
    @model_validator(mode="after")
    def validate_reference(self) -> HeldOutV2ScorecardReference:
        _validate_sha256(self.scorecard_sha256, "scorecard_sha256")
        _validate_sha256(self.case_results_sha256, "case_results_sha256")
        return self


class HeldOutV2CandidateEvidence(BaseModel):
    """One held-out v2 finalist result under the frozen policy."""
    ...
    @model_validator(mode="after")
    def validate_evidence(self) -> HeldOutV2CandidateEvidence:
        ...
        if self.variant.metadata_policy is not MetadataPolicy.AUTHORED:
            raise ValueError("held-out v2 evidence must use authored metadata policy")
        return self


class HeldOutV2Decision(BaseModel):
    """Machine-enforced held-out v2 Gate 1 decision."""
    ...
    @model_validator(mode="after")
    def validate_decision(self) -> HeldOutV2Decision:
        selected_fields = (
        ...
        if self.status is GateOneDecisionStatus.BLOCKED:
            if any(value is not None for value in selected_fields):
                raise ValueError("blocked held-out v2 decision must not include selected fields")
            if self.gate_1_passed or self.retrieval_freeze_permitted:
                raise ValueError("blocked held-out v2 decision cannot pass Gate 1")
        else:
            if any(value is None for value in selected_fields):
                raise ValueError("passed held-out v2 decision requires complete selected fields")
            if not self.gate_1_passed or not self.retrieval_freeze_permitted:
                raise ValueError("selected held-out v2 decision must pass Gate 1")
            should_confirm = (
                self.selected_retriever_config_id
            ...
            if self.status is GateOneDecisionStatus.CONFIRMED and not should_confirm:
                raise ValueError("confirmed status requires the development-v2 recommendation")
...
class HeldOutV2ValidationReport(BaseModel):
    """Hash-bound held-out v2 comparison and Gate 1 decision report."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: str = "1.0.0"
    report_id: str = "nimbus-relay-held-out-retrieval-decision-v2"
    ...
    held_out_v1_decision_path: str
    held_out_v1_decision_sha256: str
    candidate_evidence: tuple[HeldOutV2CandidateEvidence, ...] = Field(min_length=2, max_length=2)
    decision: HeldOutV2Decision

    @model_validator(mode="after")
    def validate_report(self) -> HeldOutV2ValidationReport:
        for name, value in (
        ...
        if len(config_ids) != len(set(config_ids)):
            raise ValueError("held-out v2 candidate evidence must reference unique retrievers")
        return self


class RetrievalFreezeManifestV1(BaseModel):
    """First frozen retrieval contract selected by a passed held-out v2 Gate 1."""
    ...
    @model_validator(mode="after")
    def validate_manifest(self) -> RetrievalFreezeManifestV1:
        for name, value in (
        ...
        if self.selected_metadata_policy is not MetadataPolicy.AUTHORED:
            raise ValueError("frozen retrieval must use authored metadata policy")
...
class GateOneV2Summary(BaseModel):
    """Safe CLI build or verification summary for held-out v2."""
