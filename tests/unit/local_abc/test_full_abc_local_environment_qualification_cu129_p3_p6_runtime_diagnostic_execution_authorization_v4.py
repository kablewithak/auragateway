...
module: Any = importlib.import_module(
    ...
    "p3_p6_runtime_diagnostic_execution_authorization_v4"
)

ROOT = Path(__file__).resolve().parents[3]
FIXED_NOW = datetime(2026, 8, 3, 20, 0, tzinfo=UTC)
...
def test_implementation_authority_binds_merged_assets() -> None:
    ...
    assert authority.unchanged_v3_failure_replay_authorized is False
...
def test_review_preserves_pre_execution_boundary() -> None:
    ...
    assert review.decision == "SEPARATE_TRANSIENT_SINGLE_USE_P3_P6_V4_AUTHORIZATION"
...
def test_validate_implementation_preserves_absent_authority() -> None:
    ...
    assert summary["status"] == ("P3_P6_RUNTIME_DIAGNOSTIC_EXECUTION_AUTHORIZATION_V4_VALID")
...
def test_v4_authorization_identity_and_terminal_gate() -> None:
    authorization = _authorization()

    assert authorization.scope == "P3_P6_RUNTIME_DIAGNOSTIC_V4"
    assert authorization.authorization_id.endswith("authorization-v4")
    assert module.ISSUED_NEXT_GATE == "execute_governed_p3_p6_runtime_diagnostic_v4"
    assert module.CONSUMED_NEXT_GATE == "preserve_and_accept_p3_p6_runtime_diagnostic_evidence_v4"


def test_authorization_paths_are_v4_and_untracked_by_contract() -> None:
    assert module.AUTHORIZATION_PATH.name.endswith("authorization_v4.json")
    assert module.CONSUMPTION_PATH.name.endswith("consumption_v4.json")
...
def test_v4_authorization_binds_import_closure_and_evidence_controls() -> None:
    authorization = _authorization()
...
def test_v4_terminal_gate_and_scope() -> None:
    authorization = _authorization()
...
def test_v4_confirmation_identity_is_exact() -> None:
...
def test_v4_evidence_contract_is_exact_and_fail_closed() -> None:
...
def test_v4_authorization_binds_runtime_source_and_wrapper() -> None:
    authorization = _authorization()
