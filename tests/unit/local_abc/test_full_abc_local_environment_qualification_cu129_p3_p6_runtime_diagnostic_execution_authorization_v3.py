...
module: Any = importlib.import_module(
    ...
    "p3_p6_runtime_diagnostic_execution_authorization_v3"
)
...
def test_implementation_authority_binds_merged_assets() -> None:
    ...
    assert authority.unchanged_v2_failure_replay_authorized is False
...
def test_review_preserves_pre_execution_boundary() -> None:
    ...
    assert review.decision == "SEPARATE_TRANSIENT_SINGLE_USE_P3_P6_V3_AUTHORIZATION"
...
def test_validate_implementation_preserves_absent_authority() -> None:
    ...
    assert summary["status"] == ("P3_P6_RUNTIME_DIAGNOSTIC_EXECUTION_AUTHORIZATION_V3_VALID")
...
def test_v3_authorization_identity_and_terminal_gate() -> None:
    authorization = _authorization()

    assert authorization.scope == "P3_P6_RUNTIME_DIAGNOSTIC_V3"
    assert authorization.authorization_id.endswith("authorization-v3")
...
def test_authorization_paths_are_v3_and_untracked_by_contract() -> None:
    assert module.AUTHORIZATION_PATH.name.endswith("authorization_v3.json")
    assert module.CONSUMPTION_PATH.name.endswith("consumption_v3.json")
...
def test_v3_authorization_binds_import_closure_controls() -> None:
    authorization = _authorization()
...
def test_v3_terminal_gate_and_scope() -> None:
    authorization = _authorization()
