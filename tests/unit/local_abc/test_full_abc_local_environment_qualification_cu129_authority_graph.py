...
def test_repository_authority_graph_integrates_worker_observability_harness() -> None:
    ...
    assert summary["status"] == ("CURRENT_CU129_HARNESS_EVIDENCE_INTEGRATED_AUTHORIZATION_BLOCKED")
    ...
    assert summary["fresh_authorization_base_commit_status"] == (
        "PENDING_POST_INTEGRATION_MERGE_BINDING"
    )
    assert summary["fresh_authorization_base_commit"] is None
