...
module: Any = importlib.import_module(
    ...
    "p3_p6_runtime_diagnostic_execution_authorization_v2"
)
...
def test_implementation_authority_binds_merged_assets() -> None:
    ...
    assert authority.unchanged_v1_failure_replay_authorized is False
...
def test_review_preserves_pre_execution_boundary() -> None:
    ...
    assert review.decision == "SEPARATE_TRANSIENT_SINGLE_USE_P3_P6_V2_AUTHORIZATION"
...
def test_confirmation_requires_timezone_aware_timestamp() -> None:
    with pytest.raises(ValidationError, match="timezone-aware"):
        _confirmation(confirmed_at=datetime(2026, 8, 1, 12, 0))
...
def test_validate_implementation_preserves_absent_authority() -> None:
    ...
    assert summary["status"] == ("P3_P6_RUNTIME_DIAGNOSTIC_EXECUTION_AUTHORIZATION_V2_VALID")
...
def test_v2_authorization_identity_and_terminal_gate() -> None:
    authorization = _authorization()

    assert authorization.scope == "P3_P6_RUNTIME_DIAGNOSTIC_V2"
    assert authorization.authorization_id.endswith("authorization-v2")
    assert module.ISSUED_NEXT_GATE == "execute_governed_p3_p6_runtime_diagnostic_v2"
    assert module.CONSUMED_NEXT_GATE == "preserve_and_accept_p3_p6_runtime_diagnostic_evidence_v2"


def test_authorization_paths_are_v2_and_untracked_by_contract() -> None:
    assert module.AUTHORIZATION_PATH.name.endswith("authorization_v2.json")
    assert module.CONSUMPTION_PATH.name.endswith("consumption_v2.json")
