...
module: Any = importlib.import_module(
    ...
    "p3_p6_runtime_diagnostic_execution_authorization_v1"
)

ROOT = Path(__file__).resolve().parents[3]
FIXED_NOW = datetime(2026, 7, 31, 15, 0, tzinfo=UTC)
...
def test_implementation_authority_binds_merged_assets() -> None:
    ...
    assert authority.unchanged_q6_replay_authorized is False
...
def test_review_preserves_pre_execution_boundary() -> None:
    ...
    assert review.decision == "SEPARATE_TRANSIENT_SINGLE_USE_P3_P6_AUTHORIZATION"
...
def test_confirmation_requires_timezone_aware_timestamp() -> None:
    with pytest.raises(ValidationError, match="timezone-aware"):
        _confirmation(confirmed_at=datetime(2026, 7, 31, 15, 0))
...
def test_consume_creates_single_non_reusable_receipt(
    ...
    assert receipt.saved_version_id == 339200001
...
def test_validate_implementation_preserves_absent_authority() -> None:
    ...
    assert summary["status"] == ("P3_P6_RUNTIME_DIAGNOSTIC_EXECUTION_AUTHORIZATION_V1_VALID")
