...
def test_materialization_receipt_records_expanded_archive_recovery() -> None:
...
def test_launcher_binds_integrated_harness_and_frozen_authorization_parity() -> None:
    ...
    verification = launcher.verify_launcher_notebook(
        repo_root=ROOT,
        notebook_path=ROOT / integration.LAUNCHER_NOTEBOOK_PATH,
    )
    assert verification.notebook_sha256 == integration.CURRENT_LAUNCHER_NOTEBOOK_SHA256


def test_readiness_review_keeps_authorization_unissued() -> None:
...
def test_readiness_review_rejects_incomplete_implementation_scope() -> None:
    ...
    payload["required_implementation"] = ["issue authorization"] * 8
...
def test_validator_rejects_active_materialization_record_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        integration.HarnessEvidenceIntegrationError,
        match="active manifest or materialization record does not bind the integrated harness",
    ):
        integration.validate_repository_package(repository)
